...
void draw_rays(Map *map, Player *player, Texture **texture_array, bool minimap_mode, uint32_t **pixel_buffer) {
    ...
    for (int x = 0; x <= SCREEN_WIDTH; x++) {
        ...
        texture = *(texture_array + 2);
        w = texture->get_width();
        l = texture->get_length();
        for (;y < SCREEN_LENGTH; y++) {
            double current_dist = g_lookup_table[y - 1 - (SCREEN_LENGTH >> 1)];
            double weight = current_dist * inverse_fisheye;
            double current_floor_x = (weight * dx) + x_loc;
            double current_floor_y = (weight * dy) + y_loc;
            //xdiv = (static_cast<int>(current_floor_x)) / TILE_WIDTH;
            //ydiv = (static_cast<int>(current_floor_y)) / TILE_LENGTH;
            //current_floor_x -= (xdiv * TILE_WIDTH);
            //current_floor_y -= (ydiv * TILE_LENGTH);
            current_floor_x -= ((int) current_floor_x & 0xFFFFFFE0);
            current_floor_y -= ((int) current_floor_y & 0xFFFFFFE0);
            //current_floor_x = ((int) current_floor_x % TILE_WIDTH) + (current_floor_x - (int) current_floor_x);
            //current_floor_y = ((int) current_floor_y % TILE_LENGTH) + (current_floor_y - (int) current_floor_y);
